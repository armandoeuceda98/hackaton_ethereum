...
<body>
    ...
    <div class="form">
        ...
        <form class="login-form" id="frmLogin">
            ...
            <input type="text" id="password" name="descrip" placeholder="Descripción"/>
            <label id="alerta" class="invalid-feedback"></label>
            <button id="logear" type="button">Crear</button>
        </form>
    </div>
    </div>
...
<script>
    $('#logear').on('click', function(){
        ...
        let url = "{{url('store-usuario')}}"
        var data = $('#frmLogin').serialize();
        $.post(url, data).done(function(resp) {
            console.log(resp);
            if (resp == 'El usuario ya existe.') {
                $('#alerta').text(resp);
            } else {
                alert(resp);
                location.reload();
